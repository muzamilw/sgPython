    ...
    def RunBot(self):
        ...
        while True and self.botStop.is_set() == False and RetryCount <= MaxRetryCount and IsApiClientError == False:
            try:
                ...
                if gVars.loginResult is not None:
                    ...
                    if api.authenticated_user_id is not None:
                        ...
                        try:
                            ...
                            if gVars.manifestObj.FollowOn == 1:
                            ...
                            if gVars.manifestObj.AfterFollLikeuserPosts == 1:
                            ...
                            if gVars.manifestObj.AfterFollCommentUserPosts == 1:
                        ...
                        try:
                            for i, row in islice(gVars.GlobalTodo.iterrows(),0,10000):
                                ...
                                if row['Status'] == 1 and not pd.isnull(str(row['MediaId'])) and str(row['MediaId']) != 'nan':
                                    ...
                                    if row['Action'] == 'Comment' and CommentLimitReachedPerSession == False and gVars.manifestObj.AfterFollCommentUserPosts == 1 and (gVars.CurrentCommentsDone < gVars.manifestObj.CommFollowingPosts or  gVars.CurrentExCommentsDone < gVars.ReqExComments):
                                    ...
                                    #     raise ClientError('I know Python!')
                ...
                log.info('###################################################################')
                #restarting should not refresh the manifest hence setting the following flag
                app.ManifestRefreshed = False
